...
#ifdef CONFIG_FB_MSM_MHL_SII9244  // bishin strat
...
#define FEATURE_TESTCODE_872_WWAN
...
#ifdef FEATURE_TESTCODE_872_WWAN
static char *pantech_charm_usb_functions_rmnet_sdio[] = {
	"diag",
	"diag_mdm",
	"rmnet_sdio",
};
static char *pantech_charm_usb_functions_rmnet_sdio_adb[] = {
	"diag",
	"diag_mdm",
	"adb",
	"rmnet_sdio",
...
static char *pantech_charm_usb_functions_nmea[] = {
	"diag",
	"diag_mdm",
	"nmea",
	//"rmnet_sdio", 
...
static char *pantech_charm_usb_functions_nmea_adb[] = {
	"diag",
	"diag_mdm",
	"adb",
	"nmea",
...
static struct android_usb_product charm_usb_products[] = {
	...
	},
	{
      ...
      {
        .product_id = 0x1104,
        .num_functions  = ARRAY_SIZE(pantech_charm_usb_functions_nmea),
        .functions  = pantech_charm_usb_functions_nmea,
      },
      {
        .product_id = 0x1104,
        .num_functions  = ARRAY_SIZE(pantech_charm_usb_functions_nmea_adb),
        .functions  = pantech_charm_usb_functions_nmea_adb,
      },
#ifdef FEATURE_TESTCODE_872_WWAN
	{
		.product_id	= 0x1104,
		.num_functions	= ARRAY_SIZE(pantech_charm_usb_functions_rmnet_sdio),
		.functions	= pantech_charm_usb_functions_rmnet_sdio,
	},
	{
		.product_id	= 0x1104,
		.num_functions	= ARRAY_SIZE(pantech_charm_usb_functions_rmnet_sdio_adb),
		.functions	= pantech_charm_usb_functions_rmnet_sdio_adb,
	},
#endif
 #ifdef CONFIG_USB_ANDROID_ACCESSORY
	{		
...
//PANTECH_CAMERA_TODO, fix build warning
#ifdef CONFIG_IMX074
...
static struct msm_i2c_platform_data msm_gsbi4_qup_i2c_pdata = {
    ...
    defined(CONFIG_PANTECH_CAMERA_S5K4ECGX)
	.clk_freq = 200000,
#elif defined(CONFIG_PANTECH_CAMERA_HW)
	.clk_freq = 384000,
#else
	.clk_freq = 100000,
#endif
...
#define MSM_FB_EXT_BUF_SIZE  (1920 * 1080 * 4 * 2) /* 2 bpp x 1 page */
#else
...
#ifdef CONFIG_F_SKYDISP_HDMI_CAPTION
#define MSM_FB_WRITEBACK_SIZE (1920 * 1080 * 3 * 2)
#else
...
#else
/* prim = 800 x 1280 x 4(bpp) x 2(pages)
 * hdmi = 1920 x 1080 x 2(bpp) x 2(page)
 * Note: must be multiple of 4096 */
#define MSM_FB_SIZE roundup(MSM_FB_PRIM_BUF_SIZE + 0x7E9000 + \
			MSM_FB_WRITEBACK_SIZE + MSM_FB_DSUB_PMEM_ADDER, 4096)
#endif /* CONFIG_F_SKYDISP_HDMI_CAPTION */			
#else
/* prim = 1024 x 600 x 4(bpp) x 2(pages)
...
//20111011 choiseulkee blk, disable marimba to fix warning kernel msg for EF65L bring up
...
static struct rpm_vreg_pdata rpm_vreg_init_pdata[RPM_VREG_ID_MAX] = {
	...
	RPM_VREG_INIT_LDO(PM8058_L4,  0, 1, 0, 2800000, 2800000,  LDO50HMIN, 0),
#else
...
#if defined(CONFIG_INPUT_PANTECH_EARJACK) && defined(CONFIG_MACH_MSM8X60_EF65L)
...
//	RPM_VREG_INIT_LDO(PM8058_L14, 0, 0, 0, 2850000, 2850000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L14, 0, 0, 0, 3000000, 3000000, LDO300HMIN, 0),
#ifdef CONFIG_PANTECH_CAMERA
...
#ifdef CONFIG_PANTECH_CAMERA
	RPM_VREG_INIT_LDO(PM8901_L4,  0, 1, 0, 2800000, 2800000, LDO300HMIN, 0),
#else
...
/* bucks - earmic key 구분용 mpp adc 값이 엉뚱하게 튀는 문제.... 퀄컴 패치에서 잘못된 부분 삭제
*/
#if 0
#if defined(CONFIG_INPUT_PANTECH_EARJACK) && defined(CONFIG_MACH_MSM8X60_EF65L) 
...
// --- 2011-07-28 KSJ_BT_DEVICE Bluez Initial
	...
	&msm_device_vidc,
// +++ 2011-07-28 KSJ_BT_DEVICE Bluez Initial
#if (defined(CONFIG_PANTECH_BT)) || ((defined(CONFIG_MARIMBA_CORE)) && (defined(CONFIG_MSM_BT_POWER) || defined(CONFIG_MSM_BT_POWER_MODULE)))
	&msm_bt_power_device,
#endif
#ifdef CONFIG_PANTECH_BT
...
#ifdef CONFIG_EF65L_SENSORS_APDS9900_SW
...
static int smb137b_detection_setup(void)
{
	int ret = 0;
#if defined(CONFIG_SKY_SMB136S_CHARGER) || defined(CONFIG_SKY_SMB137B_CHARGER)
//	ret = gpio_tlmm_config(GPIO_CFG(8, 0, GPIO_CFG_INPUT,	GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
#else
	ret = pm8058_mpp_config_digital_in(EXT_CHG_VALID_MPP,
					PM8058_MPP_DIG_LEVEL_S3,
					PM_MPP_DIN_TO_INT);
...
static const unsigned int ffa_keymap[] = {
	...
	// 2011.07.22 by wcjeong - wS10 Touch 絹
...
unsigned int msm8x60_sdcc_slot_status(struct device *dev)
{
	int status;
...
#define MDP_VSYNC_GPIO 28    //ejkim77 add temp
...
{
//20111011 choiseulkee blk, disable bahama to fix warning kernel msg for EF65L bring up
//#ifndef CONFIG_PANTECH
	int rc;
	int i;
	struct marimba config = { .mod_id =  SLAVE_ID_BAHAMA};
...
static unsigned max17040_config_gpio[] = {
	/* FUEL_I2C_SCL */
	GPIO_CFG(103, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	/* FUEL_I2C_SDA */
