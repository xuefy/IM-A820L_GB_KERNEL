...
#define FEATURE_SKY_SND_PANIC_SHUTDOWN
...
#ifdef FEATURE_SKY_SND_PANIC_SHUTDOWN
static int sky_snd_panic_prep(struct notifier_block *this,
				unsigned long event, void *ptr)
{
	printk(KERN_INFO "sky_snd_panic_prep : max97001 shut down\n");
...
static struct notifier_block sky_snd_panic_blk = {
	.notifier_call  = sky_snd_panic_prep,
};
#endif
...
#ifdef FEATURE_SKY_SND_PANIC_SHUTDOWN
	atomic_notifier_chain_register(&panic_notifier_list, &sky_snd_panic_blk);
#endif
...
void snd_extamp_api_SetDevice(int on, uint32_t cad_device)
{
	...
	switch(cad_device)
	{
		...
		if (on) 
		{
              ...
              /* 20111128	linecommtech - ldk	HP(L/R) vol -10dB -> -6dB */
              ...
              if(curMode==MODE_IN_CALL)	// 20111213	linecommtech ldk Headset mono(Voice) // 20111214 HP(L/R) vol -12dB -> 0dB -> -8dB
              {
                  tCurrentExtampInfo.hpl_ctl_reg_val       = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_8_DB;
                  tCurrentExtampInfo.hpr_ctl_reg_val       = HPGAIN_P_3_DB | HPRM_UNMUTE | HPRVOL_M_8_DB;
			  }
              else
              {
		...
		if (on)
		{
	          ...
	          if(curMode==MODE_IN_CALL)      
	              /* 20111012 frogLove - SP OUT volume change (-4 -> -2dB) */
			  	  /* 20111212 linecommtech - ldk	SP OUT volume change (-4 -> -2 -> -4dB) */
		...
		if (on)
		{
	          /* 20120112 frogLove - headphone gaim -3dB down(-19dB -> -22dB) */ 
	          tCurrentExtampInfo.inp_gain_ctl_reg_val  = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_3_DB;
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
	          tCurrentExtampInfo.sp_mixer_ctl_reg_val  = SPKMIX_INB2;
	          tCurrentExtampInfo.hpl_ctl_reg_val       = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_22_DB;
	          tCurrentExtampInfo.hpr_ctl_reg_val       = HPGAIN_P_3_DB | HPRM_UNMUTE | HPRVOL_M_22_DB;
	          tCurrentExtampInfo.sp_ctl_reg_val        = FFM_FF_MODE  | SPKM_UNMUTE | SPKVOL_M_4_DB;
		...
		if (on)
		{
			  ...
			  // 20111130	linecommtech ldk -12dB -> -10dB
		...
		/* 20111128	linecommtech ldk	HP(L/R) vol -12dB -> -16dB */
		if (on)
		{
	          tCurrentExtampInfo.inp_gain_ctl_reg_val  = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_0_DB;
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
	          tCurrentExtampInfo.hpl_ctl_reg_val       = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_16_DB;
	          tCurrentExtampInfo.hpr_ctl_reg_val       = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_M_16_DB;
	          tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_EN  | LPMODE_DIS | SPKEN_DIS | HPLEN_EN | HPREN_EN | BYPEN_OPEN;
		}
		else
		{
