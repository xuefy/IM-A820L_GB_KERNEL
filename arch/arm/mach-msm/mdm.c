...
#ifdef CONFIG_EF65L_MDM_RAMDMUP_ENABLE
#define RAM_DUMP_START	_IOW(0xCC, 8, int)
#define RAM_DUMP_END	       _IOW(0xCC, 9, int)
#endif
...
#ifdef CONFIG_EF65L_MDM_RAMDMUP_ENABLE
	case RAM_DUMP_START:
		printk(KERN_INFO "RAM_DUMP_START\n");
		sky_charm_ramdump_on = 1;
		break;
	case RAM_DUMP_END:
		printk(KERN_INFO "RAM_DUMP_END\n");
		msleep(10000);
		arch_reset(0, 0);
		break;
#endif
...
// paiksun...
#ifndef CONFIG_EF65L_MDM_RAMDMUP_ENABLE
#ifdef CONFIG_MACH_MSM8X60_EF65L
	boot_reason = *(unsigned int *)
		(smem_get_entry(SMEM_POWER_ON_STATUS_INFO, &smem_size));
    
        if((boot_reason == 0x10) || (boot_reason == 0x8))
        {
        	printk(KERN_INFO "Boot Reason = 0x%02x\n", boot_reason);
