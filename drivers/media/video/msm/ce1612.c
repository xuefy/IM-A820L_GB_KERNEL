...
#define F_FW_UPDATE
#if 1
#define CE1612_UPLOADER_INFO		"CE161F00_fw1722prm1722_20111129.h"
#define CE1612_UPLOADER_BIN		"CE161F01_fw1722prm1722_20111129.h"
#define CE1612_FW_INFO				"CE161F02_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_0				"CE161F03_0_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_1				"CE161F03_1_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_2				"CE161F03_2_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_3				"CE161F03_3_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_4				"CE161F03_4_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_5				"CE161F03_5_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_6				"CE161F03_6_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_7				"CE161F03_7_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_8				"CE161F03_8_fw1722prm1722_20111129.h"
#define CE1612_FW_BIN_9				"CE161F03_9_fw1722prm1722_20111129.h"
#else
...
#define CE1612_FW_MINOR_VER	22
#define CE1612_PRM_MAJOR_VER	17
#define CE1612_PRM_MINOR_VER	22
...
static int32_t ce1612_cmd_read(uint8_t cmd, uint8_t *rdata, int32_t len)
{
	...
	tmp_raddr = cmd;
      ...
      rc = ce1612_i2c_rxdata(ce1612_client->addr, &tmp_raddr, rdata, (uint16_t)len);
...
static int32_t ce1612_read(uint8_t *rdata, int32_t len)
{
	...
	rc = ce1612_i2c_rxdata_2(ce1612_client->addr, rdata, (uint16_t)len);
	if (rc < 0)
	{
...
#define CE1612_PACKETS_IN_TABLE 508
	...
	rc = ce1612_i2c_txdata(ce1612_client->addr, (uint8_t *)isp_uploader_info, CE1612_NUM_CFG_CMD);
	if (rc < 0)
	{
	...
	{
		*pFW = cmd;
		rc = ce1612_i2c_txdata(ce1612_client->addr, pFW, CE1612_NUM_UPDATE_DATA+1);
		if (rc < 0)
		{
	...
	rc = ce1612_poll(ce1612_client->addr, 0xF5, 0x05, 10, 500);
	if (rc < 0)
	{
...
//		goto fw_update_fail;
	...
	rc = ce1612_i2c_txdata(ce1612_client->addr, (uint8_t *)isp_fw_info, CE1612_NUM_CFG_CMD);
	if (rc < 0)
	{
		goto fw_update_fail;
	}
...
#endif
#if 1
	rc = ce1612_poll(ce1612_client->addr, 0xF5, 0x06, 10, 3000);
	if (rc < 0)
	{
		SKYCERR("%s : uploader polling ERROR !!!\n",__func__);
		goto fw_update_fail;
...
static int32_t ce1612_update_fw_boot(void)
{
	...
	rc = ce1612_cmd(ce1612_client->addr, 0xF0, NULL, 0);
	if (rc < 0)
	{
#if 1
//		goto update_fw_boot_done; 
// Firmware is force updated if I2C fails
		goto update_fw_boot; 
#else
		if(ce1612_client->addr == 0x3C)
			ce1612_client->addr = 0x3E;
		else
			ce1612_client->addr = 0x3C;
	...
	data = 0x00;
	rc = ce1612_cmd(ce1612_client->addr, 0x00, &data, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
//		goto update_fw_boot_done; 
...
#define CE1612_NUM_UPDATE_DATA 129
	...
	case 2:
		...
		rc = ce1612_i2c_txdata(ce1612_client->addr, paddr, CE1612_NUM_CFG_DATA+1);
	...
	case 3:
		...
		{
			*paddr = cmd;
			rc = ce1612_i2c_txdata(ce1612_client->addr, paddr, CE1612_NUM_UPDATE_DATA+1);
...
//	uint16_t revision_num = 0;
	...
	SKYCDBG("%s start : ce1612_client->addr = 0x%x\n",__func__, ce1612_client->addr);
	...
	data_buf[1] = 0x01;
	data_buf[2] = 0x02;		
	rc = ce1612_cmd(ce1612_client->addr, 0x54, data_buf, 3);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	...
	data_buf[2] = 0x00;	
	data_buf[3] = 0x20;	
	rc = ce1612_cmd(ce1612_client->addr, 0x73, data_buf, 4);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	...
	// AE/AWB enable
	data_buf[0] = 0x00;
	rc = ce1612_cmd(ce1612_client->addr, 0x11, data_buf, 1);

	// Preview start (PREVIEW)
	data_buf[0] = 0x01;
	rc = ce1612_cmd(ce1612_client->addr, 0x6B, data_buf, 1);
#endif
...
static long ce1612_set_effect(int mode, int effect)
{
	...
	if(effect == 5) //posterize
	{
		...
		rc = ce1612_cmd(ce1612_client->addr, 0x3D, data_buf, 2);	//effect off
	
		data_buf[0] = 0x07;
		data_buf[1] = 0x0E;
		rc = ce1612_cmd(ce1612_client->addr, 0x3D, data_buf, 2);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
		}	
		rc = ce1612_cmd(ce1612_client->addr, 0x01, NULL, 0);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
	...
	else
	{
		...
		data_buf[0] = 0x07;
		data_buf[1] = 0x06;
		rc = ce1612_cmd(ce1612_client->addr, 0x3D, data_buf, 2);	// gamma default		
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
...
static int32_t ce1612_set_whitebalance (int mode, int32_t whitebalance)
{
	...
	{
		data_buf[0] = 0x00;
		rc = ce1612_cmd(ce1612_client->addr, 0x1A, data_buf, 1);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x04, data_buf, 2);	
...
static int32_t ce1612_set_led_mode(int32_t led_mode)
{
	...
	if(read_data & 0x01)
	{
		rc = ce1612_cmd_poll(ce1612_client->addr, 0x35, 0, 0);	// Lens Stop
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
...
//	if((led_mode != CE1612_CFG_LED_MODE_OFF)&&(led_mode != CE1612_CFG_LED_MODE_AUTO))
	...
	switch(led_mode)
	{
		case CE1612_CFG_LED_MODE_OFF:
			...
			data_buf[0] = 0x00;
			data_buf[1] = 0x00;
			rc = ce1612_cmd(ce1612_client->addr, 0x06, data_buf, 2);
			
			data_buf[0] = 0x01;
			data_buf[1] = 0x00;
			rc = ce1612_cmd(ce1612_client->addr, 0xB2, data_buf, 2);
...
/*			data_buf[0] = 0x01;
			...
			data_buf[2] = 0x00;
			data_buf[3] = 0x00;
			rc = ce1612_cmd(ce1612_client->addr, 0xB3, data_buf, 4);
...
*/
	...
	if(read_data & 0x01)
	{
		rc = ce1612_cmd(ce1612_client->addr, 0x23, 0, 0);	// AF Start
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
...
static int32_t ce1612_set_auto_focus(int8_t autofocus)
{
	...
	switch(autofocus)
	{
		...
		case 1:	// MACRO
			data_buf[0] = 0x00;
			rc = ce1612_cmd(ce1612_client->addr, 0x42, data_buf, 1);
		
			data_buf[0] = 0x01;
			rc = ce1612_cmd_poll(ce1612_client->addr, 0x20, data_buf, 1);
			if (rc < 0)
			{
				SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
				return rc;
	...
	rc = ce1612_cmd_poll(ce1612_client->addr, 0x23, 0, 0);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_set_scene_mode(int mode, int32_t scene_mode)
{
	...
	{	
		data_buf[0] = 0x00;
		data_buf[1] = 0x00;
		rc = ce1612_cmd(ce1612_client->addr, 0x82, data_buf, 2);
...
static int32_t ce1612_set_antishake(int32_t antishake)
{
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x5B, data_buf, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_set_reflect(int32_t reflect)
{
	...
	for (i = 0; i < CE1612_POLLING_RETRY; i++) {
		rc = ce1612_i2c_read(ce1612_client->addr,
...
#define FOCUS_RECT_SIZE_1 63
	...
	if(read_data & 0x01)
	{
		...
		{
			data_buf[2] = 0x02;
			rc = ce1612_cmd(ce1612_client->addr, 0x2C, &data_buf[2], 1);
			if (rc < 0)
			{
				SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
				return rc;
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x41, data_buf, 10);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	...
	rc = ce1612_poll(ce1612_client->addr, 0x43, 0x05, 10, 1000);
	if (rc < 0)
	{
		SKYCERR("%s : uploader polling ERROR !!!\n",__func__);
		return rc;
...
static long ce1612_set_continuous_af(int caf)
{
	...
	if(caf)
	{
		...
		SKYCDBG("AF-C start\n");
		rc = ce1612_cmd(ce1612_client->addr, 0x23, 0, 0);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
			return rc;
	...
	{
#if 0
		rc = ce1612_cmd(ce1612_client->addr, 0x35, 0, 0);
#else
		rc = ce1612_cmd_poll(ce1612_client->addr, 0x35, 0, 0);
#endif
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
...
static int32_t ce1612_set_dimension(struct dimension_cfg *dimension)
{
	...
	{		
		...
		rc = ce1612_poll(ce1612_client->addr, 0x6C, 0x00, 10, 1000);
		if (rc < 0)
		{
		...
		rc = ce1612_poll(ce1612_client->addr, 0x6C, 0x08, 10, 1000);
		if (rc < 0)
		{
...
static int32_t ce1612_set_antibanding(int32_t antibanding)
{
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x14, data_buf, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_set_preview_fps(int32_t preview_fps)
{
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x5A, data_buf, 4);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_set_wdr(int32_t wdr)
{
	...
	data_buf[0] = wdr;
	rc = ce1612_cmd(ce1612_client->addr, 0x88, data_buf, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_set_caf_camera(int32_t caf)
{
	...
	data_buf[0] = 0x00;
	rc = ce1612_cmd_poll(ce1612_client->addr, 0x20, data_buf, 1); //set focus mode normal

	data_buf[0] = caf;
	rc = ce1612_cmd(ce1612_client->addr, 0x2C, data_buf, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
static int32_t ce1612_lens_stability(void)
{
	...
	data_buf[0] = 0x00;
	data_buf[0] = 0x00;
	rc = ce1612_cmd(ce1612_client->addr, 0x33, data_buf, 3);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
...
#if 1
		// stop Capture	
		rc = ce1612_cmd(ce1612_client->addr, 0x75, 0, 0);
		if (rc < 0)
		{
...
//	uint16_t read_data =0;
	...
	data_buf[1] = 0x01;
	data_buf[2] = 0x00;		
	rc = ce1612_cmd(ce1612_client->addr, 0x95, data_buf, 3);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	...
	// Data output setting
	data_buf[0] = 0x00;
	rc = ce1612_cmd(ce1612_client->addr, 0x65, data_buf, 1);
	if (rc < 0)
	{
		SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
		return rc;
	...
	rc = ce1612_cmd(ce1612_client->addr, 0x74, 0, 0);
...
#ifdef CONFIG_PANTECH_CAMERA//IRQ
{
	...
	d[2] = 0xff;
	d[3] = 0xff;
	rc = ce1612_cmd(ce1612_client->addr, 0xd1, d, 4);
	if (rc < 0) {
