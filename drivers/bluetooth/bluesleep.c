...
/* Added by HP Lee.  To meet BRCM's Bluetooth LPM requirment */
...
// [p12912- nol] - Sleep bug fix(bug: after auto headset connection, BT can't sleep)

/*#ifndef CONFIG_PANTECH_BT		
		printk(KERN_INFO"[Test_Log]%s: %d \n",__func__, gpio_get_value(bsi->ext_wake));
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	if (!test_bit(BT_TXDATA, &flags)) {
		...
		// 20110810 ksj_device for reset on repetitive BT ON/OFF
		//if(msm_hs_tx_empty(bsi->uport)){
