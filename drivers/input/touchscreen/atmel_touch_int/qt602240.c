...
#endif
	struct work_struct work_charger_mode;
...
static struct timer_list charger_mode_timer;
...
static unsigned int charger_timer_cnt = 0;
/*------------------------------ functions prototype -----------------------------------*/
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) 
	{
		...
		case TOUCH_IOCTL_CHARGER_MODE:
			...
			break;
#ifdef PROTECTION_MODE
		case TOUCH_CALL_MODE_ENABLE:
			//printk("[TSP]Touch Call Mode Enable.\n");
			//queue_work(qt602240_wq, &qt602240_data->work_protection_mode_enable);
			//mod_timer(&protection_mode_timer, jiffies + msecs_to_jiffies(60*1000));
			break;
		case TOUCH_CALL_MODE_DISABLE:
			//printk("[TSP]Touch Call Mode Disable.\n");
...
configuration_fail:
	printk("[TSP][ERROR] apply_touch_config Configuration Fail!!!");
}
...
static uint8_t qt_charger_mode_config(unsigned long mode)
{
	...
	uint8_t i = 0, finger_cnt = 0;

	dbg_func_in();

	del_timer(&charger_mode_timer);
	switch (mode)
	{
	...
	printk("[TSP] finger_cnt=%d, charger_timer_cnt=%d \n", finger_cnt, charger_timer_cnt);
	if(finger_cnt > 0 && charger_timer_cnt < 10)
	{
		charger_timer_cnt++;
		mod_timer(&charger_mode_timer, jiffies + msecs_to_jiffies(1000));
		return 0;
	}
	else if(charger_timer_cnt >= 10) {
		...
		mutex_unlock(&qt602240_data->lock);
		mod_timer(&tchautocal_timer, jiffies + msecs_to_jiffies(5000));
		charger_timer_cnt = 0;
	}
	...
	printk("[TSP] previous_charger_mode = %d \n", (int)mode);
	switch (mode)
	{
		case BATTERY_PLUGGED_NONE:
...
void  qt_charger_mode(struct work_struct * p)
{
	dbg_func_in();
	...
	for ( i= 0; i<MAX_NUM_FINGER; i++ )
	{
		if(fingerInfo[i].mode == TSC_EVENT_WINDOW)
		{
			...
			if(i == 0 && clear != TSC_CLEAR_ALL) {
				input_report_abs(qt602240_data->input_dev, ABS_MT_TOUCH_MAJOR, 1);
			}
			else {
				input_report_abs(qt602240_data->input_dev, ABS_MT_TOUCH_MAJOR, 0);
			}   
...
static void charger_mode_timer_func(unsigned long data)
{
	queue_work(qt602240_wq, &qt602240_data->work_charger_mode);
}
...
void  get_message(struct work_struct * p)
{
	...
	if (object_type == TOUCH_MULTITOUCHSCREEN_T9)
	{
		...
		if ( (quantum_msg[1] & TOUCH_RELEASE) || quantum_msg[1] & TOUCH_SUPPRESS )    
		{
			...
			touch_message_flag = 1;
			printk("[TSP] TOUCH_RELEASE || TOUCH_SUPPRESS !!\n");
		}
	...
	else if (object_type == GEN_COMMANDPROCESSOR_T6) 
	{
		...
		if(quantum_msg[1] == 0x10) {
			...
			//if (!protection_mode) {
			//	queue_work(qt602240_wq, &qt602240_data->work_protection_mode_enable);
			//	mod_timer(&protection_mode_timer, jiffies + msecs_to_jiffies(5*1000));
			//}
#endif
		}
	}
...
void report_input (void) {
	...
	if (finger_cnt == 0 && cal_correction_limit > 0) {
		//queue_work(qt602240_wq, &qt602240_data->work_protection_mode_enable);
...
static int qt602240_remove(struct i2c_client *client)
{
	...
	cancel_work_sync(&qt602240_data->work_charger_mode);
	del_timer(&charger_mode_timer);
...
static int __devinit qt602240_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_WORK(&qt602240_data->work_charger_mode, qt_charger_mode);
	init_timer(&charger_mode_timer);
	charger_mode_timer.function = charger_mode_timer_func;
	charger_mode_timer.data = 0;
