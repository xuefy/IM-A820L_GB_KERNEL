...
static struct mpu3050_platform_data mpu3050_data = {
    .int_config = 0x10,
    /* top-lower-left */
#if defined(CONFIG_EF39S_SENSORS_MPU3050) || defined(CONFIG_EF65L_SENSORS_MPU3050)
	 .orientation = { 0, -1, 0,
        -1, 0, 0,
        0, 0, -1
    },
#elif defined(CONFIG_EF40K_SENSORS_MPU3050) 
	 .orientation = { -1, 0, 0,
        0, -1, 0,
        0, 0, 1
    },
    ...
    .level_shifter = 0,
#if defined(CONFIG_EF39S_SENSORS_BMA150)
    .accel = {
	...
	(defined(CONFIG_MACH_MSM8X60_EF40K) && (BOARD_REV >= WS20))
        .adapt_num = 33, //MSM_GSBI5_QUP_I2C_BUS_ID, // The i2c bus to which the mpu device is
#else
        .adapt_num = 13, // The i2c bus to which the mpu device is
#endif
        // connected
...
#elif defined(CONFIG_EF39S_SENSORS_BMA250)
    .accel = {
...
#elif defined(CONFIG_EF40K_SENSORS_BMA150)
    .accel = {
        .get_slave_descr = bma150_get_slave_descr,
#if (defined(CONFIG_MACH_MSM8X60_EF39S) && (BOARD_REV >= TP10)) || \
	(defined(CONFIG_MACH_MSM8X60_EF40K) && (BOARD_REV >= WS20))
...
#elif defined(CONFIG_EF40K_SENSORS_BMA250)
    .accel = {
        .get_slave_descr = bma250_get_slave_descr,
#if (defined(CONFIG_MACH_MSM8X60_EF39S) && (BOARD_REV >= TP10)) || \
	(defined(CONFIG_MACH_MSM8X60_EF40K) && (BOARD_REV >= WS20))
...
#elif defined(CONFIG_EF65L_SENSORS_BMA250)
    .accel = {
        .get_slave_descr = bma250_get_slave_descr,
#if (defined(CONFIG_MACH_MSM8X60_EF65L) && (BOARD_REV >= WS10))
        .adapt_num = 33, //MSM_GSBI5_QUP_I2C_BUS_ID, // The i2c bus to which the mpu device is
#else
        .adapt_num = 13, // The i2c bus to which the mpu device is
#endif
...
#if defined(CONFIG_EF39S_SENSORS_YAS530) || defined(CONFIG_EF40K_SENSORS_YAS530) || defined(CONFIG_EF65L_SENSORS_YAS530)
    .compass = {
...
#if 0	//defined(CONFIG_EF33_BOARD) || defined(CONFIG_EF34_BOARD) || defined(CONFIG_EF35_BOARD)  || defined(CONFIG_EF40_BOARD) /*yjw*/
#define GPIO_SELECT_GYRO 154
#define GPIO_GYRO_SDA 68
#define GPIO_GYRO_SCL 69
#define GPIO_GYRO_INT 30

#define COMPASS_RESET 62
#define COMPASS_PRE_SCL 115
#define COMPASS_PRE_SDA 116

static int init_gyro_hw_setting(void)
{
	// (1-1) init compass's i2c scl
	gpio_tlmm_config(GPIO_CFG(COMPASS_PRE_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_set_value(COMPASS_PRE_SCL, 1);

	// (1-2) init compass's i2c sda
	gpio_tlmm_config(GPIO_CFG(COMPASS_PRE_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_set_value(COMPASS_PRE_SDA, 1);
	...
	// (3) init gyro's select pin
	gpio_tlmm_config(GPIO_CFG(GPIO_SELECT_GYRO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_set_value(GPIO_SELECT_GYRO, 1);

	// (4) init gyro's interrupt pin ( was don't care because of non-use )
