...
#ifdef FEATURE_ANDROID_PANTECH_USB_QC_FIX_BUG
    if(cdev->mute_switch == 1 && pantech_get_otg_chg_type() == USB_CHG_TYPE__SDP)
    {
       changed_value = get_changed_function();
       cdev->mute_switch = 0; 


	   //printk("^^^^ changed_value %d \n",changed_value);
	   //printk("^^^^ usb_product_has_rndis %d \n",usb_product_has_rndis());
	   if( changed_value == RNDIS_FUNCTION){
	   		/*do nothing*/
	   }else if( changed_value == ADB_FUNCTION){
	   		if( usb_product_has_rndis() == 0){
				/*do nothing*/	
			}else{
	   			cdev->connected = 0;
       			schedule_work(&cdev->switch_work);
