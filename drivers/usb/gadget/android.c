...
#if defined(CONFIG_USB_ANDROID_PANTECH)
...
static int __devinit android_bind(struct usb_composite_dev *cdev)
{
	...
	if(strings_dev[STRING_SERIAL_IDX].s != NULL)
	{
...
int android_enable_function(struct usb_function *f, int enable)
{
	...
	if (!!f->disabled != disable) {
		...
		if (!strcmp(f->name, "rndis")) {
			changed_function = RNDIS_FUNCTION;
		}else if (!strcmp(f->name, "adb")) {
			changed_function = ADB_FUNCTION;
...
#ifdef CONFIG_USB_ANDROID_ACCESSORY
	else if (!strncmp(f->name, "accessory", 32)){
		printk("^^^^ accessroy \n");
		android_config_functions(f, enable);
	}
#endif
#ifdef FEATURE_TESTCODE_872_WWAN
else if(!strncmp(f->name, "rmnet_sdio",32)){ 
			printk("^^^^ rmnet_sdio  enable :%d\n", enable);
			dev->cdev->desc.bDeviceClass = USB_CLASS_PER_INTERFACE;
			dev->cdev->desc.bDeviceSubClass      = 0;
			dev->cdev->desc.bDeviceProtocol      = 0;
		android_config_functions(f, enable);
		...
		else if (!strcmp(f->name, "mtp")) 
		{
			...
			device_desc.bDeviceClass =	USB_CLASS_PER_INTERFACE;
				...
				list_for_each_entry(func, &android_config_driver.functions, list){
					if(!strcmp(func->name, "modem") || !strcmp(func->name, "diag") || !strcmp(func->name, "usb_mass_storage")){
						func->disabled = 0;
					}else{
						func->disabled = 1;
					}
...
#ifdef CONFIG_USB_ANDROID_PANTECH
	    else if(!strcmp(f->name, "modem") || !strcmp(f->name, "nmea"))
	    {
   			dev->cdev->desc.bDeviceClass = 0x02;   			
         android_config_functions(f, enable);
