...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if (!host->clks_on) {
			...
			// P14774 : NEED TO DELAY
			/* Allow clear to take effect*/
			//if (host->clk_rate <= host->plat->msmsdcc_fmin)
			   msmsdcc_delay(host);
...
{
/* p14774 : 20111008 : patch for mmc resume error */
#if 0
	int rc = 0;
	...
	if (rc < 0)
		...
		return rc;
#else
//[[ 2011.10.11 leecy add qualcomm patch for mmc resume error
	int rc;
	struct device *dev = mmc->parent;
