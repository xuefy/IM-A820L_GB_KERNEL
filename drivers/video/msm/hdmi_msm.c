...
#ifndef PANTECH_HDMI_ONOFF_BUGFIX
// 20111123, kkcho, Bug fix : Movie pause->cable disconnect-> At the play before UI, qdsp state fail.
#define PANTECH_HDMI_ONOFF_BUGFIX 
// 20111222, kkcho, Bug fix : MHL Cable-in at the RGB-MODE -> HDMI-Mode change -> not display (Only LG HDMI Monitor)
#define PANTECH_HDCP_REAUTH_SUPPORT
#endif
#ifndef PANTECH_FACTORY_HDMI_TEST_INFORM // 20120105, kkcho, for HDMI_Factory_TEST #4648 
#define PANTECH_FACTORY_HDMI_TEST_INFORM 
#endif 
#ifndef PANTECH_HDMI_AUDIO_CHANGE_FIX
// 20120105, kkcho, prevent-code : 'pcm_out_write: wait_event_timeout'
// 소리가 송출되는 순간(path-change)에 HDMI_DISABLE이 호출되면 사운드 path를 잡지 못함. 
#define  PANTECH_HDMI_AUDIO_CHANGE_FIX	
#endif

#ifdef CONFIG_FB_MSM_MHL_SII9244
#define F_MHL_USB_SW_CONTROL_BUG_FIX // 20110811, kkcho, HPD로 인해 부팅시 스위치 되는 case 방지 

/*20110923, kkcho, certainly is neceesary for authentication*/
#ifndef CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT
/* AUTH for HDCP-disable*/
#define F_SKY_HDMI_HDCP_DISABLE
/* 3.2.2.3 ACP/ISRC1/ISRC2 fail modify */
#define F_SKY_HDMI_AUTH_CHECK_VALID_PACKET_TYPES
/* 3.2.2.3 AVI PACKETS */
...
//#define HDMI_RESOLUTION_DELAY_CT // 201100826, kkcho, not_used
#endif

#ifdef HDMI_RESOLUTION_DELAY_CT
struct hdmi_delay_control_via_mhl_work  {
...
#ifdef CONFIG_FB_MSM_MHL_SII9244 // 20110920, kkcho, for MHL supported
...
// PANTECH, 20111211, kkcho, Bugfix : for state halt
...
#endif
			/* Build EDID table */
#ifndef CONFIG_F_SKYDISP_HDMI_AUDIO_BUGFIX
			envp[0] = "HDCP_STATE=FAIL";
			envp[1] = NULL;
			DEV_INFO("HDMI HPD: QDSP OFF\n");
			kobject_uevent_env(external_common_state->uevent_kobj,
...
#ifdef F_SKY_HDMI_HDCP_DISABLE 
			//hdmi_msm_audio_setup();  // kkcho_0000
...
static irqreturn_t hdmi_msm_isr(int irq, void *dev_id)
{
	...
	if ((hdcp_int_val & (1 << 6)) && (hdcp_int_val & (1 << 4))) {
		...
		DEV_INFO("HDCP: AUTH_FAIL_INT received, LINK0_STATUS=0x%08x\n",
			HDMI_INP_ND(0x011C));
		if (hdmi_msm_state->full_auth_done) {
#ifdef PANTECH_HDMI_ONOFF_BUGFIX  //20111123, kkcho, Bug fix : Movie pause->cable disconnect-> At the play before UI, qdsp state fail.		
			envp[0] = "PCABLE=OFF";
			envp[1] = NULL;
			kobject_uevent_env(external_common_state->uevent_kobj,
			KOBJ_CHANGE, envp);	
			DEV_INFO("pantech_hdmi_cable_off_notify~~~~~~~~~~~~~ \n");
#endif			
			envp[0] = "HDCP_STATE=FAIL";
			envp[1] = NULL;
			DEV_INFO("HDMI HPD:QDSP OFF\n");
...
static void hdmi_msm_set_mode(boolean power_on)
{
	...
	if (power_on) {
		...
		if (external_common_state->hdmi_sink == 0) {
			/* HDMI_DVI_SEL */
			reg_val |= 0x00000002;	
#ifndef F_SKY_HDMI_HDCP_DISABLE				
			if (external_common_state->present_hdcp) 
				/* HDMI Encryption */
				reg_val |= 0x00000004;		
#endif			
...
#ifdef HDMI_RESOLUTION_DELAY_CT
extern void HDMI_Control_On(bool on); // kkcho_999

struct hdmi_delay_control_via_mhl_work hdmi_control_via_mhl;
static void hdmi_resolution_reset(struct work_struct *work)
{
       printk("[SKY_MHL] HDMI_Control_On~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	//HDMI_Control_On(1);
}
#endif
...
#ifdef HDMI_RESOLUTION_DELAY_CT
		cancel_delayed_work(&hdmi_control_via_mhl.work);	
#endif
		is_part1_done = FALSE;
		return 0;
error:
...
#ifdef HDMI_RESOLUTION_DELAY_CT 
		 schedule_delayed_work(&hdmi_control_via_mhl.work, 200);  // 5초...
#endif		
		is_part1_done = FALSE;
		return ret;
	} else {
...
#ifdef PANTECH_HDCP_REAUTH_SUPPORT
...
#ifndef F_SKY_HDMI_AUTH_CHECK_VALID_PACKET_TYPES
...
#endif /*F_SKY_HDMI_AUTH_CHECK_VALID_PACKET_TYPES*/
...
static int hdmi_msm_audio_off(void)
{
...
#ifdef PANTECH_HDMI_ONOFF_BUGFIX	 
	int i = 20; 
#else
	int i = 10; 
#endif
	...
	while (((audio_pkt_ctrl & 0x00000001) || (audio_cfg & 0x00000001))
		...
		if (!i) {
			...
			hdmi_audio_enable(0,0);  
			DEV_INFO("[kkcho]HDMI Audio: Force Disabled\n");
#endif								
			return -ETIMEDOUT;
		}
	}
...
#if (1) //def CONFIG_FB_MSM_HDMI_MSM_PANEL_HDCP_SUPPORT  // 20110826, for hdcp_disable
...
#ifdef PANTECH_HDMI_AUDIO_CHANGE_FIX
void pantech_audio_disable(void)
{
	char *envp[2];
...
EXPORT_SYMBOL(pantech_audio_disable);
#endif

#ifdef PANTECH_FACTORY_HDMI_TEST_INFORM 
void pantech_hdmi_disconnect_notify(void)
{
	char *envp[2];
	...
	kobject_uevent_env(external_common_state->uevent_kobj,
	KOBJ_CHANGE, envp);	
	DEV_INFO("pantech_hdmi_disconnect_notify_factoryTEST \n");
}
EXPORT_SYMBOL(pantech_hdmi_disconnect_notify);
#endif
...
#ifdef CONFIG_FB_MSM_MHL_SII9244  // 20110429, kkcho, MHL사용을 위해 HDMI OUTPUT format 을 낮춘다... (temp다 )	
...
#ifdef HDMI_RESOLUTION_DELAY_CT
	INIT_DELAYED_WORK_DEFERRABLE(&hdmi_control_via_mhl.work, hdmi_resolution_reset);
#endif
