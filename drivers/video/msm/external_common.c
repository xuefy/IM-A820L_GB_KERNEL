...
bool hdmi_common_get_video_format_from_drv_data(struct msm_fb_data_type *mfd)
{
	...
	if (var->reserved[3]) {
		...
		printk(KERN_ERR "[SKY_HDMI]detecting resolution from %dx%d use var->reserved[3]"
			" to specify mode", mfd->var_xres, mfd->var_yres);
		...
		case 1440:
			...
			break;
		case 1920:
#ifdef CONFIG_FB_MSM_MHL_SII9244  // 20110429, kkcho, MHL»ç¿ëÀ» À§ÇØ HDMI OUTPUT format À» ³·Ãá´Ù... (temp´Ù )	
