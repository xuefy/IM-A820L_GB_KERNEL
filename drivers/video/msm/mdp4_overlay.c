...
#ifndef CONFIG_FB_MSM_OVERLAY_WRITEBACK
/* [LS5] lived, 2011.11.08 */
#define F_SKYDISP_USE_PERF_LEVEL_UP
#endif
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
	...
	if ((mfd->panel_info.type == MIPI_VIDEO_PANEL) &&
	    (req->src_rect.h >
		req->dst_rect.h || req->src_rect.w > req->dst_rect.w)) {
#ifdef F_SKYDISP_USE_PERF_LEVEL_UP
		int ret = 0;
		ret = mdp4_overlay_validate_downscale(req, mfd,
				perf_level, (&mfd->panel_info.mipi)->dsi_pclk_rate);
		while (ret && perf_level > OVERLAY_PERF_LEVEL1) {
			perf_level--;
			//pr_info("[LIVED] perf_level up!\n");
			ret = mdp4_overlay_validate_downscale(req, mfd,
					perf_level, (&mfd->panel_info.mipi)->dsi_pclk_rate);
		};
		if (ret)
		...
		if (mdp4_overlay_validate_downscale(req, mfd,
			perf_level, (&mfd->panel_info.mipi)->dsi_pclk_rate))
#endif
#if 1//def CONFIG_FB_MSM_OVERLAY_WRITEBACK
