...
//#define MHL_POWER_ONOFF_CTRL
#endif

#ifdef MHL_POWER_ONOFF_CTRL
extern void MHL_On(bool on);
extern void HDMI_Control_On(bool on);
#endif
...
#ifdef MHL_POWER_ONOFF_CTRL
boolean Only_fist_init = FALSE;

struct sii9244_delayed_work  {
	struct delayed_work		work;
};

struct sii9244_delayed_work sii9244_delay_control;
static void sii9244_mhl_control_on(struct work_struct *work)
{
...
int mdp4_dtv_on(struct platform_device *pdev)
{
	...
	if (!Only_fist_init)
	{
		INIT_DELAYED_WORK_DEFERRABLE(&sii9244_delay_control.work, sii9244_mhl_control_on);
		Only_fist_init = TRUE;
	}

	schedule_delayed_work(&sii9244_delay_control.work, 200);  
#endif	
