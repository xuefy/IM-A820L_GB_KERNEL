...
struct pantech_cable_off_delayed_work  {
	struct delayed_work		work;
};

struct pantech_cable_off_delayed_work pantech_cable_off_notify;
#endif
	...
	printk("[SKY_MHL] MHL_En_Control  : %d\n", on);
	 ...
	 /* txs0102 control : HDMI-siganl level shift*/
        ...
        gpio_set_value(MHL_SHDN, GPIO_HIGH_VALUE);  
   	       ...
   	       gpio_set_value(MHL_WAKE_UP, GPIO_HIGH_VALUE);  //20110811, kkcho,  HPD pin이 제대로 동작 안되는 상황이므로.. 보완을 위해...	
#endif    	 
...
#ifdef F_MHL_USB_SW_CONTROL_BUG_FIX   	
   		gpio_set_value(MHL_WAKE_UP, GPIO_LOW_VALUE);  //20110811, kkcho,  HPD pin이 제대로 동작 안되는 상황이므로.. 보완을 위해...	
#endif 	
...
#ifdef PANTECH_HDMI_AUDIO_CHANGE_FIX
void pantech_hdmi_disconnect_event(void)
{
	int key = 0;
...
void pantech_hdmi_disconnect(struct work_struct *work)
{
	//cancel_delayed_work(&pantech_cable_off_notify.work);
	pantech_hdmi_disconnect_event();
}
#endif

extern void pantech_hdmi_disconnect_notify(void);  // PANTECH_FACTORY_HDMI_TEST_INFORM 
...
int MHL_Cable_On(int on)
{
	...
	if (on == prev_on)
   	...
   	{	
		...
		input_sync(ipdev);	
#else		
		pantech_hdmi_disconnect_notify(); // PANTECH_FACTORY_HDMI_TEST_INFORM 
		schedule_delayed_work(&pantech_cable_off_notify.work, 300);  
#endif		
   	}	   	
...
#ifdef PANTECH_HDMI_AUDIO_CHANGE_FIX				
			INIT_DELAYED_WORK_DEFERRABLE(&pantech_cable_off_notify.work, pantech_hdmi_disconnect);
#endif		
...
#endif
		// kkcho_1118 mhl_status_value.mhl_status = MHL_RSEN_LOW;
