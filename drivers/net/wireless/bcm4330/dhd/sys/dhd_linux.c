...
#ifdef CUSTOMER_HW_PT // suspend error ; dhdsdio_htclk: HT Avail request error: -35 
	...
	while (1)
		if (down_interruptible (&tsk->sema) == 0) {
			...
			if (dhd->pub.dongle_reset == FALSE) {
				DHD_TIMER(("%s:\n", __FUNCTION__));
#ifndef CUSTOMER_HW_PT //20110531, suspend error
 				DHD_OS_WAKE_LOCK(&dhd->pub);
#endif 
...
#ifdef CONFIG_BCM4330_IFACE_NAME
	if (!iface_name[0])
		strncpy(iface_name, CONFIG_BCM4330_IFACE_NAME, IFNAMSIZ-1);
#endif
